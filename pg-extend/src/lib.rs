...
pub(crate) unsafe fn guard_pg<R, F: FnOnce() -> R>(f: F) -> R {
    ...
    let mut local_exception_stack = mem::MaybeUninit::<pg_sys::sigjmp_buf>::uninit();
    let jumped = pg_sys::sigsetjmp(
        local_exception_stack.as_mut_ptr() as *mut _ as *mut _,
        1,
    );
    let mut local_exception_stack = local_exception_stack.assume_init();
